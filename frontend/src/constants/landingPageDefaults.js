...
const landingPageKeys = Object.keys(landingPageDefaults);

export const mergeLandingPageConfig = (config = {}) =>
  landingPageKeys.reduce((acc, key) => {
    acc[key] = typeof config?.[key] === 'string' ? config[key] : landingPageDefaults[key];
    return acc;
  }, {});
...
export const buildLandingPagePayload = (config = {}) =>
  landingPageKeys.reduce((acc, key) => {
    ...
    acc[key] = typeof value === 'string' ? value.trim() : landingPageDefaults[key];
    return acc;
  }, {});
